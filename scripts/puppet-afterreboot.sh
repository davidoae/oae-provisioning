...
##### NOTE should I do apt update && apt upgrade before running this script?
...
#chmod 1777 /tmp ##### this seems mad, removing unless it turns out to be required
cd /tmp
#wget --no-verbose http://apt.puppetlabs.com/puppetlabs-release-pc1-xenial.deb
#dpkg -i puppetlabs-release-pc1-xenial.deb

# Pull in packages from the puppetlabs repos, and clean up a little
export DEBIAN_FRONTEND=noninteractive
echo '"apt -qq update ...'
apt -qq update
echo 'apt -qq upgrade --assume-yes ...'
apt -qq upgrade --assume-yes
echo 'apt -qq autoremove --assume-yes ...'
apt -qq autoremove --assume-yes

exit

##### NOTE that at this point I switch to manual for the mo #####
...
###apt -qq --assume-yes install git puppetmaster-passenger puppetdb puppetdb-terminus libapache2-mod-passenger python-software-properties
###apt -qq --assume-yes install puppetmaster-passenger puppetdb puppetdb-termini python-software-properties
# seems at this time I can't install puppetdb-termini as it'll remove puppetmaster stuff
apt install puppetmaster-passenger puppetdb python-software-properties

# some of these cause issues on the latest version so have set versions specifically
# now we've upgraded to xenial I may be able to update some of these, for the moment they all install fine
...
##### NOTE this is different and will either be replaced or is not needed
...
# might want to be able to set option for different repo/branch
git fetch origin
bin/pull.sh

# an attempt to give the admin time to upload common_hiera_secure.json
myiptemp="$(ip a show dev eth0 | awk '/inet / { print $2 }')"
echo -e "\e[0;35mWaiting 30s to give time for admin to upload common_hiera_secure.json"
echo "Try this;"
...
##### note some of these packages no longer exist, made a best guess so far
...
apt -qq --assume-yes install build-essential ruby-mysql libmysqlclient-dev mysql-server rake ri ruby ruby-dev

##### NOTE looks like gem came with OS and now it's substantially different, so I've removed it for the moment
#####      we'll see what happens when we try to use it
...
##### NOTE can't find puppet-dashboard package :(
#apt-get -qq -y install puppet-dashboard
#####      and therefore no point in setting it up yet
#####      there's gonna be a bunch of other packages/stuff installed just for this :(
...
##### NOTE this seems pointless and systemd doesn't work like this anyway
...
##### NOTE should not need the configs unless there's something wrong with what's supplied
...
##### NOTE we currently don't have a dashboard
...
##### NOTE no dashboard at this time
...
##### NOTE commenting out all the activemq and mco stuff until I need it
#####      it's also likely that just installing mco and setting a few config items would be enough
